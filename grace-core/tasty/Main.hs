...
interpretCodeWithEnvURI = Tasty.HUnit.testCase "interpret code with env:// import" do
    ...
    Environment.setEnv "GRACE_TEST_VAR" "2 + 3"
    ...
    let expectedValue =
            Right (Type{ location, node }, Value.Scalar (Syntax.Natural 5))
          where
            location = Location{ name = uri, code = "2 + 3", offset = 0 }
...
interpretCodeWithFileURI = Tasty.HUnit.testCase "interpret code with file:// import" do
    absolute <- Directory.makeAbsolute "./tasty/data/unit/plus-input.ffg"
    ...
    let expectedValue =
            Right (Type{ location, node }, Value.Scalar (Syntax.Natural 5))
          where
            location = Location{ name = uri, code = "2 + 3\n", offset = 0 }
