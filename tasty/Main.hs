...
main = do
    ...
    let manualTestTree = Tasty.testGroup "Manual tests" [ interpretCode ]
...
interpretCode = Tasty.HUnit.testCase "interpret code" do
    actualValue0 <- Except.runExceptT (Interpret.interpret (Interpret.Code "./tasty/data/unit/plus-input.grace"))

    let expectedValue0 = Right (Type{ location, node }, Value.Natural 5)
          where
            location = Location{ name = "tasty/data/unit/plus-input.grace", code = "2 + 3\n", offset = 2 }

            node = Type.Natural

    Tasty.HUnit.assertEqual "" expectedValue0 actualValue0

    actualValue1 <- Except.runExceptT (Interpret.interpret (Interpret.Code "2 + 2"))

    let expectedValue1 = Right (Type{ location, node }, Value.Natural 4)
          where
            location = Location{ name = "(input)", code = "2 + 2", offset = 2 }

            node = Type.Natural

    Tasty.HUnit.assertEqual "" expectedValue1 actualValue1
