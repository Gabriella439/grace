...
fromJSON Type.Scalar{ scalar = Monotype.Text } (Aeson.String text) = do
    return (Value.Text (Value.Chunks text []))
...
evaluate maybeMethods = loop
  where
    ...
    loop env syntax =
        case syntax of
                ...
                let onChunk (interpolation, text₁) = do
                        ...
                        case value of
                            Value.Text (Value.Chunks text₀ []) ->
                                return (Value.Chunks (text₀ <> text₁) [])
                            v ->
                                return (Value.Chunks mempty [(v, text₁)])

                newChunks <- foldMap onChunk rest

                return (Value.Text (Value.Chunks text [] <> newChunks))
              where
            ...
            Syntax.Prompt{ schema = Just schema, ..} -> do
                ...
                case value of
                    Value.Record fieldValues
                        | Just (Value.Text (Value.Chunks prompt [])) <- HashMap.lookup "text" fieldValues -> case maybeMethods of
                            Nothing -> do
                                Exception.throwIO MissingCredentials
                            Just Methods{..} -> do
                                let model = case HashMap.lookup "model" fieldValues of
                                        Just (Value.Text (Value.Chunks m [])) -> m
                                        _ -> "o4-mini"

                                let code = case HashMap.lookup "code" fieldValues of
                                        Just (Value.Scalar (Syntax.Bool c)) -> c
                                        _ -> False
    ...
    apply (Value.Builtin RealShow) (Value.Scalar (Natural n)) =
        return (Value.Text (Value.Chunks (Text.pack (show n)) []))
    ...
    apply
      ...
      where
        ...
        inner (Value.Text c) =
            apply stringHandler (Value.Text c)
        ...
        inner (Value.Record keyValues) = do
          ...
          where
            adapt (key, value) = do
                ...
                return (Value.Record [("key", Value.Text (Value.Chunks key [])), ("value", newValue)])
        inner v =
            return v
    apply function argument =
